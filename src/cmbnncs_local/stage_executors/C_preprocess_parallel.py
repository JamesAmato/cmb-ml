...
class ParallelPreprocessExecutor(BaseStageExecutor):
    ...
    def execute(self) -> None:
        ...
        self.try_a_task(process, first_task)
        ...
        for split in self.splits:
            for sim in split.iter_sims():
                ...
                with self.name_tracker.set_contexts(contexts_dict=context):
                    cmb_asset_in = self.get_fixed_asset(self.in_cmb_asset, is_input_path=True)
                    cmb_asset_out = self.get_fixed_asset(self.out_cmb_asset, is_input_path=False)
                    x = TaskTarget(
                ...
                for freq, detector in self.instrument.dets.items():
                    context['freq'] = freq
                    with self.name_tracker.set_contexts(contexts_dict=context):
                        f_asset_in = self.get_fixed_asset(self.in_obs_assets, is_input_path=True)
                        f_asset_out = self.get_fixed_asset(self.out_obs_assets, is_input_path=False)
                        x = TaskTarget(
                            asset_in=f_asset_in,
    ...
    @staticmethod
    def get_fixed_asset(asset: Asset, is_input_path: bool):
        if is_input_path:
            assert asset.path.exists(), f"Cannot find: {asset.path}"
...
def process_map(all_map_fields: str, 
                map_data: np.ndarray, 
