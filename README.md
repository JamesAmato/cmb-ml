...
Currently, working on fixed_map_synth4.py. Trying to download all files (~63.2 GB currently). Having trouble with (timing out? it downloads for a while, then bails every time. My internet here is a turd, so I can't tell what's going on.)
